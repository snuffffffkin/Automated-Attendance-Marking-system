...
# True = record anytime for testing
# False = record only between 15:00 and 16:00
TEST_MODE = True
...
def is_class_time():
    ...
    return CLASS_START <= current_time <= CLASS_END
...
def record_event(name, event, camera):
    if not is_class_time():
        print("Outside class time. Event ignored.")
        return
    ...
    print(f"{event} recorded: {name} at {current_time} from {camera}")
...
def load_known_faces():
    ...
    for person_name in os.listdir(KNOWN_FACES_DIR):
        ...
        print(f"{person_name}: {loaded_count}/{image_count} images loaded")

    if len(known_encodings) == 0:
        print("No known faces loaded.")
        exit()

    print(f"Done loading faces! Total encodings: {len(known_encodings)}")
...
def process_frame(frame, camera_name, event_type):
    ...
    for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
        ...
        if name != "Unknown":
            print(f"Recognized: {name} from {camera_name}")
